...
<body class="product-page">
  ...
  <main id="main">
    ...
    ${relatedStories.length ? `<section class="section section--navy" aria-labelledby="related-history"><div class="shell"><div class="section-heading"><p class="eyebrow">The history behind the humor</p><h2 id="related-history">Keep exploring the story.</h2></div><div class="card-grid">${relatedStories.map((story) => `<article class="feature-card"><p class="eyebrow">${escapeHtml(story.era)}</p><h3>${escapeHtml(story.title)}</h3><p>${escapeHtml(story.summary)}</p><a href="${story.href}">${escapeHtml(story.linkLabel)}</a></article>`).join("")}</div></div></section>` : ""}
